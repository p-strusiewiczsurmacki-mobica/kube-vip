...
type endpointWorker interface {
	...
	clear(svcCtx *servicecontext.Context, lastKnownGoodEndpoint *string, service *v1.Service, leaderElectionActive *bool)
...
func (g *generic) clear(_ *servicecontext.Context, lastKnownGoodEndpoint *string, service *v1.Service, leaderElectionActive *bool) {
	g.clearEgress(lastKnownGoodEndpoint, service, leaderElectionActive)
}

func (g *generic) clearEgress(lastKnownGoodEndpoint *string, service *v1.Service, leaderElectionActive *bool) {
	if *lastKnownGoodEndpoint != "" {
		...
		*lastKnownGoodEndpoint = "" // reset endpoint
		if g.config.EnableServicesElection || g.config.EnableLeaderElection {
			g.leaseMgr.Delete(service) // stop services watcher
		}
