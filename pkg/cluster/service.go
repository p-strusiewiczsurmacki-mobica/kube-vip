...
func CountRouteReferences(serviceInstances []*Instance, route *netlink.Route) int {
	log.Info("CountRouteReferences findng", "route", route)
	cnt := 0
	for _, instance := range serviceInstances {
		for _, cluster := range instance.Clusters {
			for n := range cluster.Network {
				hasEndpoints := cluster.Network[n].HasEndpoints()
				log.Info("CountRouteReferences", "has endpoints", hasEndpoints, "ip", cluster.Network[n].IP())
				if cluster.Network[n].HasEndpoints() {
					r := cluster.Network[n].PrepareRoute()
					log.Info("CountRouteReferences existing", "route", r)
