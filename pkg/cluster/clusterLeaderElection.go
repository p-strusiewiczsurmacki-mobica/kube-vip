...
func (cluster *Cluster) StartCluster(ctx context.Context, c *kubevip.Config, sm *Manager, bgpServer *bgp.Server, leaseMgr *lease.Manager) error {
	var err error
	...
	objectName := fmt.Sprintf("%s/%s-cp", ns, c.LeaseName)
	objLease, newLease, sharedLease := leaseMgr.Add(c.LeaseName, objectName)
	...
	if !newLease {
		...
		case <-ctx.Done():
			// Service was deleted
			leaseMgr.Delete(c.LeaseName, objectName)
	...
	if sharedLease {
		...
		log.Debug("cluster waiting for context done", "lease", c.LeaseName)
		// Block until context is cancelled
		<-ctx.Done()

		log.Debug("cluster waiting for lease done", "lease", c.LeaseName)
...
func (cluster *Cluster) onStartedLeadingAction(ctx context.Context, c *kubevip.Config, sm *Manager, bgpServer *bgp.Server, leaderCancel context.CancelFunc, signalChan chan os.Signal) {
	...
	err := cluster.vipService(ctx, c, sm, bgpServer, leaderCancel)
	if err != nil {
