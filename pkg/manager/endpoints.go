...
func (p *Processor) AddModify(svcCtx *serviceContext, event watch.Event,
	...
	leaderCtx *context.Context, leaderCancel *context.CancelFunc) (bool, error) {
	...
	if len(endpoints) != 0 {
		...
		p.updateLastKnownGoodEndpoint(lastKnownGoodEndpoint, endpoints, service, leaderElectionActive, *leaderCancel)

		// start leader election if it's enabled and not already started
		if !*leaderElectionActive && p.sm.config.EnableServicesElection {
			*leaderCtx, *leaderCancel = context.WithCancel(svcCtx.ctx)
			go startLeaderElection(*leaderCtx, p.sm, leaderElectionActive, service)
		}
