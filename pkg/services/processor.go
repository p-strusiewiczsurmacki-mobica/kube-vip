...
func (p *Processor) AddOrModify(ctx context.Context, event watch.Event, serviceFunc *Callback, wg *sync.WaitGroup) (bool, error) {
	...
	if len(svcAddresses) <= 0 && len(svcHostnames) <= 0 {
		...
		if err != nil {
			return true, fmt.Errorf("failed to get updated LB addresses for service %s/%s: %w", svc.Namespace, svc.Name, err)
		}
		svc = s
	}
	...
	if svcInstance == nil {
		...
		if err != nil {
			return false, fmt.Errorf("unalbe to create instance for service %s/%s", svc.Namespace, svc.Name)
		}
