...
func GetLogs(ctx context.Context, client kubernetes.Interface, tempDirPath string) error {
	...
	if err := getLogsFromDir(intCtx, tempDirPath); err != nil {
...
func getLogsFromDir(ctx context.Context, dir string) error {
	...
	for _, c := range containers {
		if (strings.Contains(c.Names[0], "kube-vip") || strings.Contains(c.Names[0], "services")) &&
