...
func CreateCluster(ctx context.Context, spec *ClusterSpec) *Cluster {
	...
	Eventually(e2e.LoadDockerImageToKind(spec.Logger, spec.KubeVIPImage, spec.Name), "20s").Should(Succeed())

	c.Logger.Printf("Loading traefik image into nodes")
	Eventually(e2e.LoadDockerImageToKind(spec.Logger, "ghcr.io/traefik/whoami:v1.11", spec.Name), "20s").Should(Succeed())
